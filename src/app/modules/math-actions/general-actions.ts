...
export function getChildren(struct: MathStruct): MathStruct[] {
    ...
    function get(struct: MathStruct): void {
...
export function toMultiplier(struct: MathStruct): Multiplier {
    if (struct instanceof Term) {
        return struct.sign == "+" && struct.content.length == 1 ?
            struct.content[0].copy() :
            new Expression([struct.copy()]);
...
export function toExpression(struct: Multiplier | Term, sign: "+" | "-" = "+"): Expression {
    ...
    if (struct instanceof Term) {
        if (struct.sign == "+" && struct.content.length == 1 && struct.content[0] instanceof Expression) {
            return struct.content[0].copy();
        }
